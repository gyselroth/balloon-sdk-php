...
/**
 * BalloonAppSharelinkV2Api Class Doc Comment
 *
 * @category Class
 * @package  Balloon\Sdk
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class BalloonAppSharelinkV2Api
{
    /**
     * @var ClientInterface
     */
    ...
    public function createLink($node, $expiration = '0', $password = null)
    {
        ...
        try {
            try {
                $response = $this->createLinkRequest($node, $expiration, $password);
     ...
     */
    public function createLinkRequest($node, $expiration = '0', $password = null)
    {
        $request = $this->createLinkPrepareRequest($node, $expiration, $password);
     ...
     */
    protected function createLinkPrepareRequest($node, $expiration = '0', $password = null)
    {
    ...
    public function deleteLink($node)
    {
        ...
        try {
            try {
                $response = $this->deleteLinkRequest($node);
            } catch (RequestException $e) {
                throw new ApiException(
                    "[{$e->getCode()}] {$e->getMessage()}",
                    $e->getCode(),
     ...
     */
    public function deleteLinkRequest($node)
    {
        $request = $this->deleteLinkPrepareRequest($node);
        return $this->client->send($request);
    }
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function deleteLinkPrepareRequest($node)
    {
        // verify the required parameter 'node' is set
        if ($node === null || (is_array($node) && count($node) === 0)) {
            throw new \InvalidArgumentException(
